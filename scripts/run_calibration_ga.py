#!/usr/bin/env python
"""
Calibration-set training optimisation via Genetic Algorithm.

Given a target island and calibration split, select the best subset of
training data (by island groups or by individuals) that maximises
...
This mirrors the Shapley split protocol (same cal/test split, same
evaluation metrics) but replaces TMC-Shapley with a GA search.

Outputs
-------
``calibration_ga_results.csv`` – one row per (target, cal_setting, repeat,
n_train_size, method) with columns:
    n_train_size, n_individuals, corr_eval_cal, corr_eval_test,
    mse_adj_test, fitness, method, selected_islands, target_island,
    target_island_name, n_cal_samples, repeat, repeat_seed, trait

Usage
-----
    python -m scripts.run_calibration_ga --config config/calibration_ga_config.json
"""
...
def _resolve_shard_assignment(
    ...
    if not n_train_sizes_list:
        raise ValueError(
            "Sharded execution requires calibration_ga.n_train_sizes to be set "
...
def run_merge(config_path: Path) -> None:
    ...
    ga_section = cfg.get("calibration_ga", cfg.get("island_ga", {}))
    output_dir = Path(
        ga_section.get("output_dir", cfg["paths"].get("output_dir", "outputs/calibration_ga"))
    )
    ...
    for trait_spec in trait_specs:
        ...
        for csv_path in sorted(shards_root.glob(f"shard_*/{trait_name}/calibration_ga_results.csv")):
            if csv_path.exists():
        ...
        out_path = trait_output / "calibration_ga_results.csv"
...
def run_individual_ga(
    ...
    """
    Run GA selecting individuals directly to maximise corr_cal / n_individuals.
    """
    n_source = int(X_source.shape[0])

    def fitness_fn(ind_subset: np.ndarray) -> float:
        cal_res = _evaluate_individual_subset_on_cal(
            train_indices=ind_subset,
            X_source=X_source,
            y_source=y_source,
            X_cal=X_cal,
            y_eval_cal=y_eval_cal,
        ...
        n_ind = int(cal_res["n_individuals"])
        if n_ind == 0:
            return float("inf")
        return -(float(cal_res["corr_eval_cal"]) / n_ind)

    best_subset, best_fitness, ga_stats = run_ga(
        n_candidates=n_source,
        n_train=int(n_train_individuals),
        fitness_fn=fitness_fn,
        cfg=ga_cfg,
    )

    cal_res = _evaluate_individual_subset_on_cal(
        train_indices=best_subset,
        X_source=X_source,
        y_source=y_source,
        X_cal=X_cal,
        y_eval_cal=y_eval_cal,
    ...
    return {
        "best_indices": best_subset.tolist(),
        "corr_eval_cal": float(cal_res["corr_eval_cal"]),
        "corr_eval_test": float(test_res["corr_eval_test"]),
        "mse_adj_test": float(test_res["mse_adj_test"]),
        "n_individuals": int(test_res["n_individuals"]),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Calibration-set training optimisation via GA"
    )
    parser.add_argument(
        "--mode",
    ...
    parser.add_argument(
        "--config",
        required=True,
        help="Path to calibration_ga_config.json",
    )
    parser.add_argument(
        "--target_islands",
        nargs="+",
    ...
    if model_type != "ridge":
        raise ValueError("calibration_ga currently only supports model.type='ridge'")
    ridge_alpha = float(model_cfg.get("alpha", 1e5))

    selection_mode = str(ga_section.get("selection_mode", "island")).lower()
    if selection_mode not in {"island", "individual"}:
        raise ValueError("calibration_ga.selection_mode must be 'island' or 'individual'")

    # n_train_sizes to sweep over (islands in island mode, individuals in individual mode)
    n_train_sizes_list = _as_int_list(
        ga_section.get("n_train_sizes", ga_section.get("n_train_islands", None)), default=[]
    )
    ...
    assigned_repeat_idx, assigned_n_train = _resolve_shard_assignment(
        n_repeats=n_repeats,
        n_train_sizes_list=n_train_sizes_list,
    ...
    for trait_spec in trait_specs:
        ...
        results_path = trait_output / "calibration_ga_results.csv"
        ...
        for target_code in target_codes:
            ...
            # Determine n_train_size sweep
            if not n_train_sizes_list:
                if selection_mode == "island":
                    sweep_sizes = list(range(1, n_source + 1))
                else:
                    raise ValueError(
                        "In selection_mode='individual', calibration_ga.n_train_sizes must be provided."
                    )
            else:
                max_source_individuals = int(np.sum((locality != target_code) & np.isin(locality, source_codes)))
                max_size = n_source if selection_mode == "island" else max_source_individuals
                sweep_sizes = [k for k in n_train_sizes_list if k <= max_size]

            if assigned_n_train is not None:
                sweep_sizes = [k for k in sweep_sizes if int(k) == int(assigned_n_train)]
                if len(sweep_sizes) == 0:
                    logger.info(
                        "Target %s (%s): assigned n_train_size=%d exceeds available max=%d; skipping target for this shard.",
                        target_code,
                        target_name,
            ...
            logger.info(
                ...
                "selection_mode=%s, n_train_sizes sweep: %s, n_cal_samples: %s, repeats: %d",
                target_code,
                target_name,
                n_source,
            ...
            for n_cal in n_cal_samples_list:
                ...
                for repeat_idx in repeat_indices:
                    ...
                    # --- Full-training baseline (all source individuals) ---
                    if selection_mode == "island":
                        ...
                        all_selected_islands = str(source_codes)
                        all_selected_names = str([source_names.get(c, str(c)) for c in source_codes])
                    else:
                        all_ind_idx = np.arange(int(X_source_filt.shape[0]), dtype=np.int64)
                        all_cal_res = _evaluate_individual_subset_on_cal(
                            train_indices=all_ind_idx,
                            X_source=X_source_filt,
                            y_source=y_source_filt,
                            X_cal=X_cal,
                            y_eval_cal=y_eval_cal,
                        ...
                        all_test_result = _evaluate_individual_subset_on_test(
                            train_indices=all_ind_idx,
                            X_source=X_source_filt,
                            y_source=y_source_filt,
                            X_test=X_test,
                        ...
                        all_cal_corr = float(all_cal_res["corr_eval_cal"])
                        all_n_ind = int(all_cal_res["n_individuals"])
                        all_selected_islands = "[]"
                        all_selected_names = "[]"

                    all_fitness = (float(all_cal_corr) / int(all_n_ind)) if int(all_n_ind) > 0 else 0.0

                    all_row = pd.DataFrame([{
                        "n_train_size": int(all_n_ind),
                        "n_train_islands": int(len(source_codes)) if selection_mode == "island" else -1,
                        ...
                        "selected_islands": all_selected_islands,
                        "selected_island_names": all_selected_names,
                    ...
                    # ---- Per n_train_size ------------------------------------
                    for n_train_size in sweep_sizes:
                        n_train_size = int(n_train_size)
                        max_train = n_source if selection_mode == "island" else int(X_source_filt.shape[0])
                        logger.info(
                            "  n_train_size=%d/%d (mode=%s) ...",
                            n_train_size, max_train, selection_mode,
                        )

                        ga_seed = int((repeat_seed + n_train_size * 7919) % 2_147_483_647)
                        ga_cfg = GAConfig(
                            ...
                            n_swaps_per_mut=min(ga_n_swaps, max(1, n_train_size - 1)),
                            seed=ga_seed,
                            verbose=ga_verbose,
                        ...
                        if selection_mode == "island":
                            ...
                            selected_islands = ga_result["best_islands"]
                            selected_names = [source_names.get(c, str(c)) for c in selected_islands]
                        else:
                            ga_result = run_individual_ga(
                                n_train_individuals=n_train_size,
                                X_source=X_source_filt,
                                y_source=y_source_filt,
                                X_cal=X_cal,
                                y_eval_cal=y_eval_cal,
                        ...
                        logger.info(
                            "  GA done (%.1fs): mode=%s, n_ind=%d, corr_cal=%.4f, corr_test=%.4f, fitness=%.6f",
                            elapsed,
                            selection_mode,
                        ...
                        ga_row = pd.DataFrame([{
                            "n_train_size": n_train_size,
                            "n_train_islands": n_train_size if selection_mode == "island" else -1,
                            ...
                            "selection_mode": selection_mode,
                            "selected_islands": str(selected_islands),
                            ...
                            "ga_cache_size": ga_result["ga_stats"].get("cache_size", 0),
                            "ga_elapsed_sec": ga_result["ga_stats"].get("elapsed_sec", elapsed),
                        }])
                        ...
                        if selection_mode == "island":
                            ...
                            for rr in rand_results:
                                rand_names = [source_names.get(c, str(c)) for c in rr["selected_islands"]]
                            ...
                            rng_rand = np.random.default_rng(ga_seed + 1)
                            rand_results = _random_individual_baseline_matched_n(
                                n_individuals_match=n_train_size,
                                X_source=X_source_filt,
                                y_source=y_source_filt,
                                X_cal=X_cal,
                                y_eval_cal=y_eval_cal,
                        ...
                        logger.info(
                            "  Baselines: random (%d) mean corr_test=%.4f | all_individuals corr_test=%.4f",
                            n_random_orders,
                            np.mean([r["corr_eval_test"] for r in rand_results]) if len(rand_results) > 0 else float("nan"),
