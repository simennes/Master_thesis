#!/usr/bin/env python
"""
PCA-on-source training-set selection and weighting.

Conceptually equivalent to avg-GRM top-k but uses a PCA distance metric:
  1. Compute PCA on all source (non-target) individuals.
  2. Project target-island individuals into that PC space and compute
     the target centroid.
  3. Distance = Euclidean distance from each source individual to the
     target centroid in the source PC space.
...
-----
Worker:
  python -m scripts.run_pca_source_rank_weight --mode worker --config config/pca_source_rank_weight_config.json

Merge shards:
  python -m scripts.run_pca_source_rank_weight --mode merge --config config/pca_source_rank_weight_config.json
"""
...
def _make_repeat_seed(global_seed: int, target_code: int, repeat_idx: int) -> int:
    token = f"pca_source|{int(global_seed)}|{int(target_code)}|{int(repeat_idx)}"
...
def _parse_n_components_values(exp_cfg: Dict[str, Any]) -> List[int]:
    ...
    if not values:
        raise ValueError("pca_source_experiment.n_components must contain at least one positive integer")
    return values


def _parse_alpha_values(exp_cfg: Dict[str, Any]) -> List[float]:
    """Accept None, float, or list-like config for distance-threshold alphas."""
    raw = exp_cfg.get("alpha_values", exp_cfg.get("alpha_distances", None))
    if raw is None:
        return []
    ...
    values = sorted(set(v for v in values if float(v) > 0))
    if not values:
        return []
    return values


def _parse_apriori_rule_cfg(exp_cfg: Dict[str, Any]) -> Dict[str, Any]:
    raw = exp_cfg.get("apriori_rule", {})
    if raw is None:
        raw = {}
    if not isinstance(raw, dict):
        raise ValueError("pca_source_experiment.apriori_rule must be an object")

    out: Dict[str, Any] = {
        "enabled": bool(raw.get("enabled", True)),
        "c90_factor": float(raw.get("c90_factor", 1.05)),
        "diversity_factor": float(raw.get("diversity_factor", 0.90)),
        "near_full_fraction": float(raw.get("near_full_fraction", 0.98)),
    }

    if out["c90_factor"] <= 0:
        raise ValueError("apriori_rule.c90_factor must be > 0")
    if out["diversity_factor"] <= 0:
        raise ValueError("apriori_rule.diversity_factor must be > 0")
    if out["near_full_fraction"] <= 0:
        raise ValueError("apriori_rule.near_full_fraction must be > 0")

    return out


def _mean_pairwise_euclidean(X_sub: np.ndarray) -> float:
    n = int(X_sub.shape[0])
    if n < 2:
        return 0.0
...
def _apriori_geometry_curves(
    Z_source: np.ndarray,
    Z_target: np.ndarray,
    order: np.ndarray,
    candidate_ks: np.ndarray,
) -> pd.DataFrame:
    if len(candidate_ks) == 0:
        return pd.DataFrame(columns=["k", "c90", "diversity"])

    candidate_ks = np.array(sorted(int(k) for k in np.unique(candidate_ks) if int(k) > 0), dtype=np.int64)
    nearest = np.full(Z_target.shape[0], np.inf, dtype=float)
    prev_k = 0
    rows: List[Dict[str, Any]] = []

    for k in candidate_ks:
        k_int = int(k)
        new_idx = order[prev_k:k_int]
        if len(new_idx) > 0:
            d_block = cdist(Z_target, Z_source[new_idx], metric="euclidean")
            nearest = np.minimum(nearest, np.min(d_block, axis=1))

        selected_idx = order[:k_int]
        c90 = float(np.quantile(nearest, 0.90)) if nearest.size > 0 else float("nan")
        diversity = _mean_pairwise_euclidean(Z_source[selected_idx])
        rows.append({
            "k": k_int,
            "c90": c90,
            "diversity": diversity,
...
# ---------------------------------------------------------------------------
# PCA-on-source distance
# ---------------------------------------------------------------------------

def compute_pca_source_distances(
    X_source: np.ndarray,
    X_target: np.ndarray,
    n_components: int,
) -> Tuple[np.ndarray, np.ndarray, np.ndarray, float]:
    """
    Fit PCA on source (training) genotypes, project target individuals
    into that PC space, compute their centroid, then return:
      - Euclidean distance of each source individual to that target centroid.
      - Target spread (median distance of target individuals to their centroid).
    """
    n_components = min(n_components, X_source.shape[0], X_source.shape[1])
    pca = PCA(n_components=n_components)
    Z_source = pca.fit_transform(X_source)   # (n_source, n_components)
    Z_target = pca.transform(X_target)       # (n_target, n_components)
    target_centroid = Z_target.mean(axis=0)  # (n_components,)
    distances_source = np.linalg.norm(Z_source - target_centroid, axis=1)
    distances_target = np.linalg.norm(Z_target - target_centroid, axis=1)
    target_spread = float(np.median(distances_target)) if len(distances_target) > 0 else 1.0
    return Z_source, Z_target, distances_source, target_spread
...
def run_merge(config_path: Path) -> None:
    ...
    exp_cfg = cfg.get("pca_source_experiment", {})
    output_dir = Path(exp_cfg.get("output_dir", cfg["paths"].get("output_dir", "outputs/pca_source_rank_weight")))
    ...
    for trait_spec in trait_specs:
        ...
        merge_specs = [
            ("pca_source_rank_weight_results.csv", True),
            ("pca_source_ranked_selected_individuals.csv", False),
            ("pca_source_weighted_individuals.csv", False),
            ("pca_source_apriori_diagnostics.csv", False),
        ]
        ...
        if merged_results is not None and len(merged_results) > 0:
            # Fill NA alpha values (for non-alpha-based methods)
            if "alpha" not in merged_results.columns:
                merged_results["alpha"] = np.nan
            else:
                merged_results["alpha"] = merged_results["alpha"].fillna(np.nan)
            
            summary = (
                merged_results.groupby(
                    ["trait", "target_island", "target_island_name", "analysis", "method", "n_components", "alpha", "n_individuals"],
                    dropna=False,
                    as_index=False,
                )
                .agg(
            ...
            summary_csv = trait_output / "pca_source_rank_weight_summary.csv"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="PCA-on-source ranked + weighted training-set analysis")
    ...
    alpha_values = _parse_alpha_values(exp_cfg)
    apriori_rule_cfg = _parse_apriori_rule_cfg(exp_cfg)
    ...
    for trait_spec in trait_specs:
        ...
        results_path = trait_output / "pca_source_rank_weight_results.csv"
        selected_path = trait_output / "pca_source_ranked_selected_individuals.csv"
        weights_path = trait_output / "pca_source_weighted_individuals.csv"
        apriori_diag_path = trait_output / "pca_source_apriori_diagnostics.csv"
        for p in [results_path, selected_path, weights_path, apriori_diag_path]:
            if p.exists():
                p.unlink()
        ...
        for target_code in target_codes:
            ...
            # The full-source fit is stored separately as full_baseline, so skip k == n_source.
            step_counts = step_counts[step_counts < n_source]
            steps_by_target[int(target_code)] = step_counts

            for repeat_idx in range(n_repeats):
                # One job per (target, repeat). This avoids repeating baseline/random work per n_components.
                per_repeat_weight = (
                    1
                    + (len(step_counts) * (1 + n_random_reps))
                    + (len(n_components_values) * (len(step_counts) + len(alpha_values) + len(weight_schemes)))
                )
                jobs.append({
                    "target_code": int(target_code),
                    "repeat_idx": int(repeat_idx),
                    "weight": float(max(1, n_source) * per_repeat_weight),
                })
        ...
        for target_code in target_codes:
            ...
            for repeat_idx in range(n_repeats):
                ...
                logger.info(
                    ...
                    done, total, trait_name, target_code, repeat_idx + 1, n_repeats,
                )

                # Full-source baseline is independent of PCA settings: write once per target/repeat.
                ...
                full_row = {
                    ...
                    "alpha": np.nan,
                    "c90_geom": np.nan,
                    "diversity_geom": np.nan,
                    "apriori_triggered_early": np.nan,
                    "apriori_k_selected": np.nan,
                }
                _append_csv(pd.DataFrame([full_row]), results_path)

                # Random baselines are also independent of PCA settings: write once per target/repeat.
                ...
                for n_components in n_components_values:
                    # --- PCA on source, project target, compute distances ---
                    Z_source, Z_target, distances, target_spread = compute_pca_source_distances(
                        X_source, X_target, int(n_components)
                    )
                    ...
                    # Distance-threshold alpha-based analysis (if alpha values configured)
                    if alpha_values:
                        for alpha in alpha_values:
                            threshold = alpha * target_spread
                            chosen = np.where(distances <= threshold)[0]
                            ...
                            row = {
                                "analysis": "distance_threshold",
                                "method": "pca_source_alpha",
                                "weight_scheme": "none",
                                "order_seed": -1,
                                "weighted_fit_used": False,
                                "n_individuals": int(n_selected),
                                "corr_eval": float(eval_result["corr_eval"]),
                                "mse_adj": float(eval_result["mse_adj"]),
                                "pca_dist_obj": float(np.mean(distances[chosen])) if n_selected > 0 else float("nan"),
                                "n_components": int(n_components),
                                "target_island": int(target_code),
                                "target_island_name": str(target_name),
                                "repeat": int(repeat_idx),
                    ...
                    # A-priori geometry-based k selection (independent of prediction labels)
                    if bool(apriori_rule_cfg.get("enabled", True)):
                        apriori_ks = np.unique(np.concatenate([step_counts, np.array([N_source], dtype=np.int64)]))
                        apriori_ks = np.array(sorted(int(k) for k in apriori_ks if int(k) > 0), dtype=np.int64)

                        curves = _apriori_geometry_curves(
                            Z_source=Z_source,
                            Z_target=Z_target,
                            order=order,
                            candidate_ks=apriori_ks,
                        )

                        if len(curves) > 0:
                            c90_full = float(curves.loc[curves["k"] == int(N_source), "c90"].iloc[0])
                            d_full = float(curves.loc[curves["k"] == int(N_source), "diversity"].iloc[0])

                            c90_limit = float(apriori_rule_cfg["c90_factor"]) * c90_full
                            d_limit = float(apriori_rule_cfg["diversity_factor"]) * d_full

                            near_full_frac = float(apriori_rule_cfg["near_full_fraction"])
                            k_near_full = int(max(1, np.floor(near_full_frac * N_source)))

                            curves = curves.copy()
                            curves["rule_satisfied"] = (
                                (curves["c90"] <= c90_limit) & (curves["diversity"] >= d_limit)
                            )

                            eligible = curves[(curves["rule_satisfied"]) & (curves["k"] <= k_near_full)]
                            if len(eligible) > 0:
                                selected_k = int(eligible["k"].min())
                                triggered_early = bool(selected_k < N_source)
                            else:
                                selected_k = int(N_source)
                                triggered_early = False

                            chosen = order[:selected_k]
                            eval_result = _evaluate_ridge_subset(
                                train_idx=chosen,
                                X_source=X_source,
                                y_source=y_source,
                            ...
                            selected_row = curves[curves["k"] == selected_k].iloc[0]
                            apriori_row = {
                                "analysis": "apriori_geometry",
                                "method": "pca_source_apriori_topk",
                                "weight_scheme": "none",
                                "order_seed": -1,
                                "weighted_fit_used": False,
                                "n_individuals": int(selected_k),
                                "corr_eval": float(eval_result["corr_eval"]),
                                "mse_adj": float(eval_result["mse_adj"]),
                                "pca_dist_obj": float(np.mean(distances[chosen])) if selected_k > 0 else float("nan"),
                                "n_components": int(n_components),
                                "target_island": int(target_code),
                                "target_island_name": str(target_name),
                                "repeat": int(repeat_idx),
                                ...
                                "trait": trait_name,
                                "alpha": np.nan,
                                "c90_geom": float(selected_row["c90"]),
                                "diversity_geom": float(selected_row["diversity"]),
                                "apriori_triggered_early": bool(triggered_early),
                                "apriori_k_selected": int(selected_k),
                            }
                            _append_csv(pd.DataFrame([apriori_row]), results_path)

                            apriori_selected_df = pd.DataFrame({
                                "trait": trait_name,
                                "target_island": int(target_code),
                                "target_island_name": str(target_name),
                                "repeat": int(repeat_idx),
                                "repeat_seed": int(repeat_seed),
                                "n_train_size": int(selected_k),
                                "method": "pca_source_apriori_topk",
                                "n_components": int(n_components),
                                "alpha": np.nan,
                                "ringnr": ids_source[chosen],
                            ...
                            _append_csv(apriori_selected_df, selected_path)

                            diag_df = curves.copy()
                            diag_df["trait"] = trait_name
                            diag_df["target_island"] = int(target_code)
                            diag_df["target_island_name"] = str(target_name)
                            diag_df["repeat"] = int(repeat_idx)
                            diag_df["repeat_seed"] = int(repeat_seed)
                            diag_df["n_components"] = int(n_components)
                            diag_df["c90_full"] = float(c90_full)
                            diag_df["diversity_full"] = float(d_full)
                            diag_df["c90_limit"] = float(c90_limit)
                            diag_df["diversity_limit"] = float(d_limit)
                            diag_df["selected_k"] = int(selected_k)
                            diag_df["selected_row"] = (diag_df["k"] == int(selected_k))
                            diag_df["triggered_early"] = bool(triggered_early)
                            _append_csv(diag_df, apriori_diag_path)
                    ...
                    for scheme_cfg in weight_schemes:
                        ...
                        row = {
                            ...
                            "method": f"pca_source_weight_{scheme_name}",
