...
def main() -> None:
    ...
    logger.info(
        "Sweep config: n_permutations=%s, cal_fractions=%s, repeats=%d, targets=%s",
        n_perm_grid,
        cal_frac_grid,
        n_repeats,
        target_codes,
    ...
    split_plan_path = sweep_output / "target_split_plan.csv"
    fixed_snp_path = sweep_output / "fixed_snp_columns.npy"
    ...
    # Fixed SNP subset for entire sweep (constant across all cells)
    ...
    if use_snp_selection and num_snps is not None and int(num_snps) < int(X.shape[1]):
        ...
        logger.info("Fixed SNP subset selected once for full sweep: n=%d saved to %s", len(fixed_snp_cols), fixed_snp_path)

    # Pre-draw and store target split plans for each (target, cal_frac, repeat)
    ...
    total_jobs = len(target_codes) * len(n_perm_grid) * len(cal_frac_grid) * n_repeats
    done_jobs = 0
            ...
            for n_perm in n_perm_grid:
                for repeat_idx in range(n_repeats):
                    ...
                    if save_cell_artifacts:
                        run_output_dir = sweep_output / f"target_{target_code}" / f"perm_{n_perm}" / f"cal_{cal_frac:.3f}" / f"repeat_{repeat_idx}"
                    else:
                        run_output_dir = sweep_output / "tmp"

                    logger.info(
                        "Sweep %d/%d | target=%s perm=%d cal=%.3f repeat=%d/%d",
                        done_jobs + 1,
                        total_jobs,
                        target_code,
                        n_perm,
    ...
    if not remove_rows:
        raise RuntimeError("No successful sweep runs produced remove-curve outputs.")
    ...
    logger.info("Sweep complete. Outputs written to: %s", sweep_output)
